...
balance_bridge_umee=$(${COS_BIN_NAME} q bank balances ${COS_BR_ADDR} --node "tcp://127.0.0.1:${COS_PORT_RPC}" -o json | jq  '.balances[] | select(.denom=="'${COS_DENOM}'") | .amount | tonumber')
